...
public:
    ...
    void agregarArista(int u, int v, int costo, int tiempo) {
        ...
        adj[v].push_back({u, costo, tiempo});
    }
    ...
    void mostrarEstructura() {
        ...
        for (int i = 1; i <= vertices; i++) {
            for (auto& a : adj[i]) {
                if (i < a.destino) { // Mostrar una sola vez (No dirigido)
        ...
        if (!tieneAristas) cout << "  (El grafo no tiene aristas aun)" << endl;
        cout << "===================================\n" << endl;
    }
    ...
    // Guardar en formato JSON (Sin comas finales huérfanas)
    void guardarJSON(string nombreArchivo) {
        ...
        f << "\n  ]\n}";
        f.close();
        cout << "Grafo guardado exitosamente en '" << nombreArchivo << "'" << endl;
    }

    // Cargar desde archivo JSON parseando las líneas manualmente
    void cargarJSON(string nombreArchivo) {
        ifstream f(nombreArchivo);
        if (!f.is_open()) {
            cout << "AVISO: No se encontro '" << nombreArchivo << "'. Se iniciara con un grafo vacio." << endl;
            return;
        }
        ...
        if (max_v > 0) {
            ...
            cout << "Grafo cargado desde archivo con " << max_v << " vertices." << endl;
        }
    }
};

// Funcion para pedir los datos de una arista de forma limpia
void pedirYAgregarAristas(Grafo &g) {
...
int main() {
    string archivo = "grafo.json";
    Grafo g;

    // Al arrancar se leen los datos previos del JSON
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 4: {
                int v;
                cout << "\n[Creando Nuevo Grafo] ¿Cual es el numero estimado de vertices iniciales?: ";
                cin >> v;
                g.inicializar(v);
                pedirYAgregarAristas(g);
                break;
